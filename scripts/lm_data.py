...
import os, collections, sys, subprocess
...
class charNGramData(LMData):
	...
	def build_skipgram_vocab(self, filename, skip):
		...
		# find all skip-grams
		for word in data:
			#print(u'word: {0}'.format(word).encode(self.encoding))
			...
			if word in self.special_symbols:
				...
				#print(u'first_skipgram: {0}'.format(first_skipgram).encode(self.encoding))
				skipgrams.add(first_skipgram)
				...
				for pos in xrange(len(word)):
					...
					if len(word[pos:]) >= skip+2:
						...
						# add skipgram if not yet in set
						skipgrams.add(curr_skipgram)
		...
		for skipgram, freq in freq_skipgrams.iteritems():
			print('{0}\t{1}'.format(skipgram, freq))

		# if only skipgrams with a frequency > cutoff have to be kept
		if 'skipgram_cutoff' in self.config:
			freq_skipgrams = {skipgram:freq for skipgram, freq in freq_skipgrams.items() if freq > self.config['skipgram_cutoff']}

		# unknown n-gram symbol
		freq_skipgrams['<UNKskipgram>'] = len(freq_skipgrams)

		skipgrams = freq_skipgrams.keys()

		self.skipgram_to_id = dict(zip(skipgrams, range(len(skipgrams))))
		self.id_to_skipgram = dict(zip(range(len(skipgrams)), skipgrams))

		#print('self.skipgram_to_id: {0}'.format(self.skipgram_to_id))
		print('len self.skipgram_to_id: {0}'.format(len(self.skipgram_to_id)))

		# special symbol to indicate whether the word contains (a) capital(s) or not
		if 'capital' in self.config:
			self.skipgram_to_id['<cap>'] = len(self.skipgram_to_id)
			self.id_to_skipgram[len(self.id_to_skipgram)] = '<cap>'
	...
	def file_to_skipgram_ids(self, filename, skip):
		...
		for word in data:
			...
			skipgram_repr = np.zeros(len(self.skipgram_to_id), dtype=np.float32)

			# do not cut word in n-grams if it contains only 1 character or is a special symbol
			if len(word) == 1 or word in self.special_symbols:
				if word in self.skipgram_to_id:
					skipgram_repr[self.skipgram_to_id[word]] += 1
				else:
					skipgram_repr[self.skipgram_to_id['<UNKskipgram>']] += 1

			else:

				# append '<bow>' to beginning of word
				first_skipgram = '<bow>'+word[skip]
				if first_skipgram in self.skipgram_to_id:
					skipgram_repr[self.skipgram_to_id[first_skipgram]] += 1
				else:
					skipgram_repr[self.skipgram_to_id['<UNKskipgram>']] += 1
				...
				if 'capital' in self.config:
					...
					if num_capitals > 0:
						# increase count at index of special capital marker
						skipgram_repr[self.skipgram_to_id['<cap>']] += num_capitals
				...
				for pos in xrange(len(word)):
					...
					if len(word[pos:]) >= skip+2:
						...
						if curr_skipgram in self.skipgram_to_id:
							skipgram_repr[self.skipgram_to_id[curr_skipgram]] += 1
							#print('skipgram in vocab: index = {0}'.format(self.skipgram_to_id[curr_skipgram]))
						else:
							skipgram_repr[self.skipgram_to_id['<UNKskipgram>']] += 1
							#print('skipgram not in vocab: index = {0}'.format(self.skipgram_to_id['<UNKskipgram>']))

				# WHAT ABOUT THE END OF THE WORD?
				last_skipgram = word[-1-skip]+'<eow>'
				#print(u'last_skipgram: {0}'.format(last_skipgram).encode(self.encoding))

				if last_skipgram in self.skipgram_to_id:
					skipgram_repr[self.skipgram_to_id[last_skipgram]] += 1
				#	print('skipgram in vocab: index = {0}'.format(self.skipgram_to_id[last_skipgram]))
				else:
					skipgram_repr[self.skipgram_to_id['<UNKskipgram>']] += 1
				#	print('skipgram not in vocab: index = {0}'.format(self.skipgram_to_id['<UNKskipgram>']))
	...
	def read_data(self):
		# n-gram input: use data where words are not converted to <UNK>
		...
		if 'skipgram' in self.config:
			self.build_skipgram_vocab(train_path_full_vocab, self.config['skipgram'])
		else:
	...
	def get_data(self):
		all_data = self.read_data()
		if 'skipgram' in self.config:
			lengths = (len(self.id_to_skipgram), len(self.id_to_item))
		else:
			lengths = (len(self.id_to_ngram), len(self.id_to_item))
		print('lenghts: {0}'.format(lengths))
	...
	def get_batch(self):
		...
		#if ((self.iterator * num_steps) + num_steps + 1) >= batch_len:
		#	print('change y')
		#	y = data_array_words[:, (self.iterator * num_steps) +1 : ]
