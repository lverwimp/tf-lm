...
class inputLM(object):
	...
	def __init__(self, config, data, name=None):
		...
		if 'per_sentence' in config:
			data = [word for sentence in data for word in sentence] # flatten list of lists
...
def print_samples(input_sample, target_sample, id_to_word):
	...
	for row in input_sample:
		for col in row:
			#if not id_to_word[col] == '<bos>' and not id_to_word[col] == '<eos>' and not id_to_word[col] == '@': #remove this
...
def main(_):
	...
	with tf.Graph().as_default():
		...
		with tf.name_scope("Test"):
			...
			with tf.variable_scope("Model", reuse=True, initializer=initializer):
				mtest = LM(is_training=False, config=eval_config,input_=test_input)
